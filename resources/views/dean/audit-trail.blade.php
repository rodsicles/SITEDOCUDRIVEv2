...
@section('content')
    ...
    {{-- Recent Document Reads --}}
    <div class="content-card mt-6">
        <div class="card-header">
            <h3 class="card-title">
                <i class="fas fa-file-alt mr-2 text-[#028a0f] dark:text-[#02b815]"></i>
                Recent Document Reads
                <span class="badge badge-info ml-2">Last 20</span>
            </h3>
        </div>
        <div class="overflow-x-auto">
            <table class="data-table compact" style="width: 100%;">
                ...
                <tbody>
                    @forelse($recentDocReads as $read)
                        <tr>
                            <td class="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {{ $read->viewed_at->format('M d, Y') }}<br>
                                <span class="text-gray-400 dark:text-gray-500">{{ $read->viewed_at->format('h:i:s A') }}</span>
                            </td>
                            <td>
                                <strong class="text-sm text-gray-800 dark:text-gray-200">
                                    {{ optional(optional($read->user)->employee)->full_name ?? optional($read->user)->username ?? '—' }}
                                </strong>
                            </td>
                            <td class="text-sm text-gray-700 dark:text-gray-300 break-words">
                                {{ optional($read->document)->document_title ?? '—' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-8 text-gray-500 dark:text-gray-400">
                                <i class="fas fa-file-alt text-2xl mb-2 block text-gray-300 dark:text-gray-600"></i>
                                No document reads recorded yet.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="content-card mt-6">
        ...
        <div class="overflow-x-auto">
            <table class="data-table compact" style="width: 100%;">
                ...
                <tbody>
                    @forelse($recentAnnouncementReads as $read)
                        <tr>
                            <td class="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {{ $read->read_at->format('M d, Y') }}<br>
                                <span class="text-gray-400 dark:text-gray-500">{{ $read->read_at->format('h:i:s A') }}</span>
                            </td>
                            <td>
                                <strong class="text-sm text-gray-800 dark:text-gray-200">
                                    {{ optional(optional($read->user)->employee)->full_name ?? optional($read->user)->username ?? '—' }}
                            ...
                            <td class="text-sm text-gray-700 dark:text-gray-300 break-words">
                                {{ optional($read->announcement)->title ?? '—' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-8 text-gray-500 dark:text-gray-400">
                                <i class="fas fa-bullhorn text-2xl mb-2 block text-gray-300 dark:text-gray-600"></i>
                                No announcement reads recorded yet.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
