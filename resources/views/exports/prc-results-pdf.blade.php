...
/* ── Summary table (Examination / Passed / Total) ── */
.summary-table {
    width: 100%;
...
.summary-table td.col-exam { width: 56%; text-align: left; }
.summary-table td.col-num  { width: 22%; text-align: center; }
...
<div class="doc-title">PRC Board Examination Results</div>
<div class="doc-subtitle">A.Y. {{ now()->year }}-{{ now()->year + 1 }}</div>
<div class="doc-period"><u>{{ strtoupper($batchLabel) }}</u></div>

<div class="section-heading">Examination Summary</div>
<table class="summary-table">
    <thead>
        <tr>
            <th class="col-exam" style="text-align:left;">Examination</th>
            <th class="col-num">Passed</th>
            <th class="col-num">Total Examinees</th>
        </tr>
    </thead>
    <tbody>
        @foreach($examData as $exam)
        <tr>
            <td class="col-exam">{{ $exam['exam_type'] }}</td>
            <td class="col-num">{{ $exam['passed_count'] }}</td>
            <td class="col-num">{{ $exam['total_examinees'] ?? 'N/A' }}</td>
        </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr>
            <td class="col-exam">Total</td>
            <td class="col-num">{{ $totalPassed }}</td>
            <td class="col-num">{{ $totalExaminees > 0 ? $totalExaminees : 'N/A' }}</td>
        </tr>
    </tfoot>
</table>
    ...
    <table class="passers-table">
        ...
        <tbody>
            @foreach($examData as $exam)
                ...
                <tr class="group-row">
                    <td colspan="2">{{ $exam['exam_type'] }}</td>
                </tr>
                    @foreach($exam['passer_names'] as $i => $name)
                    <tr>
